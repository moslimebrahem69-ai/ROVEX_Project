...
class SplashScreen extends StatefulWidget {
  final bool autoBoot;
  const SplashScreen({super.key, this.autoBoot = true});
...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _boot() async {
    ...
    unawaited(AmbientAudio.instance.start());
    if (!mounted) return;

    setState(() {
  _progress = 0.35;
  _loadingMessage = 'Loading Machine Interface...';
});
    ...
    setState(() {
  _progress = 0.62;
  _loadingMessage = 'Preparing Precision Control Modules...';
});

    final deadline = DateTime.now().add(_maxWait);
    while (mounted && DateTime.now().isBefore(deadline)) {
      if (svc.status.connected) break;
    ...
    final remain = _minSplash - elapsed;
    ...
    Navigator.of(context).pushReplacement(
      PageRouteBuilder(
        pageBuilder: (_, __, ___) => const HmiShellScreen(),
