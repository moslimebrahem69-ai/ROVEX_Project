...
# 4. Run a deterministic preflight review before loading the path
python -m edge_ai.review_cli \
   --pattern examples/optimized_path.csv \
   --report examples/design_review.json

# Optional visual review through OpenRouter (never controls the machine)
# PowerShell: $env:OPENROUTER_API_KEY = "your-key"
python -m edge_ai.review_cli \
   --pattern examples/optimized_path.csv \
   --image path/to/design.png \
   --gcode path/to/design.gcode \
   --ai \
   --report examples/design_review_ai.json
...
# 7. Execute the path against the simulated hardware backend
