...
/// Converts an ASCII DXF drawing into an ordered ROVEX tufting path.
///
/// Supported entities:
/// - LINE
/// - CIRCLE
...
///
/// DXF layers are preserved as separate color/thread groups.
/// The first appearance of a layer determines its processing order.
class DxfExtractor {
  static const int _maxTotalPoints = 6000;
  static const double _arcSegmentDegrees = 6.0;
  static const double _epsilon = 1e-9;

  /// Parses DXF bytes and returns a stitch path.
  ...
  /// Parses a DXF string and returns a stitch path.
  static ExtractResult extractFromString(
    ...
    if (workWidthMm <= 0 || workHeightMm <= 0) {
      return const ExtractResult(
        points: [],
        colors: [],
      );
    ...
    for (final group in layerGroups.values) {
      if (group.polylines.isEmpty) {
        continue;
      }
      ...
      final orderedPolylines = _chainNearest(
        group.polylines,
      );
  ...
  static String _decode(Uint8List bytes) {
    ...
    // DXF ASCII files are normally UTF-8/ASCII.
    // allowMalformed keeps the parser robust against legacy files.
    try {
      return utf8.decode(
        bytes,
        allowMalformed: true,
  ...
  /// Groups entities by DXF layer while preserving first appearance order.
  ...
  /// Calculates a global sampling stride so huge DXF files don't generate
  /// an excessive number of stitch points.
  static int _samplingStride(
    List<_DxfEntity> entities,
  ) {
    ...
    if (rawPointCount <= _maxTotalPoints) {
      return 1;
    }

    return math.max(
      1,
      (rawPointCount / _maxTotalPoints).ceil(),
    );
  }
  ...
  static void _appendPolyline(
    ...
    for (var i = 0; i < polyline.length; i += stride) {
      final point = transform.toMillimeters(
        polyline[i],
      );
    ...
    // Always preserve the exact geometric endpoint.
    final last = transform.toMillimeters(
      polyline.last,
    );
  ...
  /// Orders separate polylines using a greedy nearest-neighbor strategy.
  ///
  /// The endpoint closest to the current cursor is selected. If the end
  /// endpoint is closer, the polyline is reversed.
  static List<List<List<double>>> _chainNearest(
    ...
    if (polylines.length == 1) {
      return List<List<List<double>>>.from(
        polylines,
      );
    }

    final remaining = List<List<List<double>>>.from(
      polylines,
    );
  ...
  /// Resolves the DXF ACI color when available.
  ///
  /// If the entity/layer does not provide a supported ACI color,
  /// a deterministic generated color is used.
  static int _colorFor(
    int index,
    int? aci,
  ) {
  ...
  static int _hsvToRgb(
    ...
    final r = ((red + match) * 255)
        .round()
        .clamp(0, 255);

    final g = ((green + match) * 255)
        .round()
        .clamp(0, 255);

    final b = ((blue + match) * 255)
        .round()
        .clamp(0, 255);
  ...
  /// Basic AutoCAD Color Index palette.
...
/// Tokenizes a DXF into (group code, value) pairs.
class _DxfTokenizer {
  static List<MapEntry<int, String>> tokenize(
    String text,
  ) {
...
/// Reads supported entities from the DXF ENTITIES section.
class _DxfEntityReader {
  ...
  List<_DxfEntity> read() {
    ...
    while (index < range.end) {
      ...
      switch (type) {
        ...
        default:
          // Unsupported entities are intentionally ignored.
          break;
      }
    }
  ...
  _TokenRange _findEntitiesRange() {
    ...
    var foundEntitiesSection = false;

    for (var i = 0; i < tokens.length; i++) {
      if (tokens[i].key == 2 &&
          tokens[i].value.toUpperCase() == 'ENTITIES') {
        start = i + 1;
        foundEntitiesSection = true;
        break;
      }
    }

    if (!foundEntitiesSection) {
      // Some non-standard exporters omit SECTION/ENTITIES.
      // In that case scan the complete token list.
      start = 0;
    }

    for (var i = start; i < tokens.length; i++) {
      if (tokens[i].key == 0 &&
          tokens[i].value.toUpperCase() == 'ENDSEC') {
        end = i;
        break;
      }
    }
  ...
  void _addCircle(
    ...
    final steps = math.max(
      12,
      (360.0 / DxfExtractor._arcSegmentDegrees)
          .ceil(),
    );
  ...
  void _addArc(
    ...
    final startDegrees = _number(block, 50) ?? 0;
    final endDegrees = _number(block, 51) ?? 0;
    ...
    // DXF ARC runs counter-clockwise.
    if (end < start) {
      end += 2 * math.pi;
    }
    ...
    final steps = math.max(
      ...
      (sweep.abs() *
              180.0 /
              math.pi /
              DxfExtractor._arcSegmentDegrees)
          .ceil(),
    );
  ...
  _DxfEntity _readLwPolyline(
    ...
    for (final token in block) {
      switch (token.key) {
        ...
        case 42:
          currentBulge =
              double.tryParse(token.value) ?? 0.0;
          break;
      }
    }
  ...
  /// Reads classic POLYLINE followed by VERTEX/SEQEND records.
  int _readPolyline(
    List<_DxfEntity> entities,
    List<MapEntry<int, String>> block,
    int index,
    ...
    // Classic POLYLINE can contain a closed flag (70).
    final flags = _number(block, 70)?.toInt() ?? 0;
    ...
    while (index < end) {
      ...
      if (type == 'VERTEX') {
        ...
        final vertexBlock = tokens.sublist(
          vertexStart,
          index,
        );
  ...
  _DxfEntity _verticesToEntity({
    ...
    final segmentCount =
        closed ? vertices.length : vertices.length - 1;
  ...
  /// Converts a DXF bulge value into sampled arc points.
...
/// Converts DXF coordinates into the ROVEX work area.
...
/// Global geometric bounds of all DXF entities.
class _Bounds {
  const _Bounds({
    required this.minX,
    required this.minY,
...
/// A group of DXF entities belonging to one layer.
class _LayerGroup {
  _LayerGroup({
    required this.layer,
...
/// Internal DXF entity representation.
class _DxfEntity {
  _DxfEntity({
    required this.layer,
    required this.colorIndex,
...
/// A classic POLYLINE vertex.
class _PolylineVertex {
  const _PolylineVertex(
    this.x,
    this.y,
    this.bulge,
  );
...
/// Simple 2D point used by the coordinate transform.
class _Point {
  const _Point(
    this.x,
    this.y,
...
/// Global function used by MachineService isolates.
...
/// Compatibility alias used by existing MachineService code.
