...
class MachineService extends ChangeNotifier {
  ...
  double get previewSpeedPercent =>
      status.feedOverride.clamp(10, 200).toDouble();
  ...
  Future<void> loadPathFile(
    ...
    final previousPoints = List<TuftPoint>.from(_programPoints);
    ...
    if (lines != null) {
      ...
      for (final line in lines.skip(1)) {
        ...
        loadedPoints.add(TuftPoint(x: x, y: y));
      }

      if (loadedPoints.isNotEmpty) {
        if (previousPoints.length == loadedPoints.length) {
          _programPoints = List.generate(
            loadedPoints.length,
            (index) => TuftPoint(
              x: loadedPoints[index].x,
              y: loadedPoints[index].y,
              colorValue: previousPoints[index].colorValue,
              colorOrder: previousPoints[index].colorOrder,
            ),
          );
        } else {
  ...
  void _beginLocalSeg() {
    ...
    final speedMultiplier =
        _status.feedOverride.clamp(10, 200) / 100.0;
    final speedMmPerSecond =
        _simulationBaseSpeedMmPerSecond * speedMultiplier;
